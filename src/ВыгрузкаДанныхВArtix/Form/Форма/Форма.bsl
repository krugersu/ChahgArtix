...
Процедура СформироватьШаблонФайлаНаСервере()
	...
	НомерНагазина = "Обмен с Artix";
	СтруктураНоменклатура = ПолучитьСтруктуруНоменклатуры();

	
	ИмяФайлаВыгрузки = "H:\Пашков\Задачи\"+НомерНагазина+"\All.aif";
	
	РезультатЗапросаПоНоменклатуре = ВыполнитьЗапросПоНоменклатуре();
...
Функция ВыполнитьЗапросПоНоменклатуре()
	...
	|	Номенклатура.ВесЧислитель КАК taracapacity,
...
Процедура СоздатьФайлФлаг(НомерМагазина)
	...
	ИмяФайлаВыгрузки = "H:\Пашков\Задачи\"+ НомерМагазина + "\All.flz";
