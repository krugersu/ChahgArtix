		...
		Если НЕ РезультатКассоваяСмена.Количество() = 0 Тогда
			...
			Если текКассоваяСмена <> Неопределено Тогда
				...
				Если  НеФормироватьЧеки Тогда
					...
					//ТаблицаШапкиЧеков = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode);
		...
		Если НЕ РезультатКассоваяСмена.Количество() = 0 Тогда
			...
			Если текКассоваяСмена <> Неопределено Тогда
				...
				Если  НеФормироватьЧеки Тогда
					...
					СписокИДЧековВСмене.Очистить();
					//ТаблицаШапкиЧеков = ЗагрузитьЧекиНаСервере(текКассоваяСмена.НомерСменыККТ);
					//ТаблицаШапкиЧеков = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode);//TODO РезультатКассоваяСмена[0].cashcode - переделать
					ТаблицаШапкиЧековПродажи = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode,1);
					ТаблицаШапкиЧековВозвраты = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode,25);
				...
				Иначе // Загружаем чеки и потом на их основе формируем ОРП 
					СписокИДЧековВСмене.Очистить();
					//ТаблицаШапкиЧеков = ЗагрузитьЧекиНаСервере(текКассоваяСмена.НомерСменыККТ,РезультатКассоваяСмена[0].cashcode);
					ТаблицаШапкиЧековПродажи = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode,1);
					ТаблицаШапкиЧековВозвраты = ЗагрузитьЧекиНаСервере(Число(текКассоваяСмена.Номер),РезультатКассоваяСмена[0].cashcode,25);
...
Функция СформироватьОРП(текКассоваяСмена,ТаблицаТЧТовары,СписокИДЧековВСмене,КодКассы,ВидДокумента)
	...
	СсылкаНаНовыйДокумент  = СформироватьШапкуОРП(текКассоваяСмена,ТаблицаТЧТовары,ВидДокумента);
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция СформироватьШапкуОРП(текКассоваяСмена,ТаблицаТЧТовары,ВидДокумента)
...
Функция ПолучитьЗафиксироватьДатуОпроса()
	...
	СтароеВремя = Константы.крю_ВремяПоследнегОпроса.Получить(); 
	Константы.крю_ВремяПоследнегОпроса.Установить(ТекущаяДата());
