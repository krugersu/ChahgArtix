...
Функция ОткрытьКассовуюСмену(Результат)
	...
	ВыборкаДетальныеЗаписи.Следующий();
			...
			Попытка
				...
				СоздатьИзменитьЗаписьОКассовойСмене(тСтрока.cashcode,СсылкаНаКассовуюСмену.НомерСменыККТ,Статус);
...
Функция ПолучитьСписокНеЗакрытыхКассовыхСменВ1С(ДатаПоследнегоОпроса)
	...
	НомераКассАртикс = Запрос.Выполнить().Выгрузить();
	
	Для каждого тНомерКассыАртикс Из НомераКассАртикс Цикл
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"
		|ВЫБРАТЬ
		|	крю_КассовыеСмены.КассаКомплексная КАК КассаКомплексная,
		|	крю_КассовыеСмены.КассаАртикс КАК КассаАртикс,
		|	крю_КассовыеСмены.НомерКассовойСмены КАК НомерКассовойСмены,
		|	крю_КассовыеСмены.СтатусКассовойСмены КАК СтатусКассовойСмены
		|ИЗ
		|	РегистрСведений.крю_КассовыеСмены КАК крю_КассовыеСмены
		|ГДЕ
		|	крю_КассовыеСмены.КассаАртикс = &КассаАртикс";
		
		Запрос.Параметры.Вставить("КассаАртикс", Строка(тНомерКассыАртикс)); // <Строка>[9999]
		
		НомерКассовойСмены = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("НомерКассовойСмены");
		...
		|ГДЕ
		|	workshift.shiftnum > &shiftnum
		|	И workshift.cashcode В(&cashcode)";
		
		Запрос.УстановитьПараметр("cashcode", тНомерКассыАртикс);
		Запрос.УстановитьПараметр("shiftnum", Строка(НомерКассовойСмены));
...
Функция ЗакрытьКассовуюСмену(текСмена,РезультатКассоваяСмена);
	...
	Если НЕ ТекущаяКассоваяСмена = Документы.КассоваяСмена.ПустаяСсылка() Тогда
		...
		Попытка
			...
			СоздатьИзменитьЗаписьОКассовойСмене(РезультатКассоваяСмена[0].cashcode,ТекущаяКассоваяСмена.НомерСменыККТ,Перечисления.СтатусыКассовойСмены.Закрыта);
...
Процедура СоздатьИзменитьЗаписьОКассовойСмене(КассаАртикс,НомерСмены,Статус)
	...
	НаборЗаписей.Отбор.КассаАртикс.Установить(КассаАртикс); 
