...
Функция ПолучитьСписокНеЗакрытыхКассовыхСменВ1С(ДатаПоследнегоОпроса)
	...
	// Список смен в 1С начина с даты последенго опроса,  выгружанм номера документов в список значений
		Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Для Каждого СтрокаТаблицыИсточник Из СписокКассовыхСмен Цикл
		
		ЗаполнитьЗначенияСвойств(СменыКассовыйСервер.Добавить(), СписокКассовыхСмен);
